...
class Chatbot_formation():
    ...
    def makeGlobalResearch(self,msg,number,dom=None):
        ...
        values = torch.cosine_similarity(emb,self.AllEmbeddings[AllIndex])
        similarities = values.numpy()
        
        # Best score
        pos = similarities.argmax()
        best_score = similarities[pos]
        ...
        for i in range(number):
            ...
            formations = formations + formation_found + "\n" #" --> " + str(score) +"\n"
    ...
    def getAllEmbeddings(self):
        # Make preprocessing
        tmp_dataframe = self.formationsDB["intitule"].apply(Preprocessing)
        
        # Get all the titles of formations
        intitules = tmp_dataframe.tolist()
        ...
        tokens = self.tokenizer(intitules,padding=True,truncation=True,max_length=self.max_length,return_tensors="pt")
        with torch.no_grad():
            h = self.model(**tokens)
        emb = mean_pooling(h,tokens["attention_mask"],normalize=True)
        return emb
