...
class Chatbot():
    ...
    def getResponse(self,msg):
        ...
        if self.AskingProfilResearch is not None:
            ...
            formations_found,best_score = self.chatbot_formation.makeGlobalResearch(self.AskingProfilResearch,8,self.userProfil)
    ...
    def IntentsHandler(self,intent,msg):
        ...
        if intent == "AskForFormation":
            ...
            # If the profil of the user is known, we can make the search of training
                ...
                if result is None:
                    result = msg
                # We set the number of training returned to 8
                formations_found,best_score = self.chatbot_formation.makeGlobalResearch(result,8,self.userProfil)
                response = choice(FormationOfferMessages) + formations_found
