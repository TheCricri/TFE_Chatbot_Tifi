...
def rescaleSTSB(lang="fr"):
    train_ds = load_dataset("stsb_multi_mt", name=lang, split="train")
    valid_ds = load_dataset("stsb_multi_mt", name=lang, split="dev")
    test_ds = load_dataset("stsb_multi_mt", name=lang, split="test")
...
class PAWSTrainingModel(nn.Module):
    ...
    def forward(self,input_ids1=None,attention_mask1=None,input_ids2=None,attention_mask2=None,labels=None):
        ...
        # Use mean pooling
        u = mean_pooling(h1,attention_mask1,normalize=True)
        v = mean_pooling(h2,attention_mask2,normalize=True)
...
class XNLITrainingModel(nn.Module):
    def __init__(self,body,label=3,d_model=768):
        super(XNLITrainingModel, self).__init__()
...
if __name__ == "__main__":
    ...
    model_save1 = "Model_SentenceEmbedding/Finetuning/distilcamembert-base1-1epoch"
    model_save2 = "Model_SentenceEmbedding/Finetuning/distilcamembert-base2-1epoch"
    model_save3 = "Model_SentenceEmbedding/Finetuning/distilcamembert-base3-2epoch"
    batch_size = 16
    warmup = 0.1
    learning_rate = 2e-5
    weight_decay = 0.01
    ...
    for epoch in range(num_epochs):
        ...
        for batch in trainloader:
            ...
            # Add the loss
            tmp_loss_train.append(loss.item())
            #Update the count
            count = count + 1
        ...
        # Make the loss independant to the batch size
        tmp_loss_train = sum(tmp_loss_train)/len(trainloader)
        # Append the epoch training loss
        loss_train.append(tmp_loss_train)
    ...
    # # Transfrom into the form : ['sentence1','sentence2',label']
    # train = train.rename_column("premise","sentence1")
    # train = train.rename_column("hypothesis","sentence2")
    # validation = validation.rename_column("premise","sentence1")
    # validation = validation.rename_column("hypothesis","sentence2")
    # test = test.rename_column("premise","sentence1")
    # test = test.rename_column("hypothesis","sentence2")
    ...
    # # Load the model used as body
    # body = AutoModel.from_pretrained(model_save2,max_length=max_length)
    ...
    # plt.title("STSB Dataset training")
    ...
    ##
    ## Testing the models on STSB test set
    ##
    ...
    # # Model name to test
    # model_test = "Model_SentenceEmbedding/Finetuning/distilcamembert-base3-2epoch"
    
    # # Load the model used as body
    # body = AutoModel.from_pretrained(model_test,max_length=max_length)
    ...
    #     # Predict the batch
    #     with torch.no_grad():
    #         _,similarity = model(**batch)
    #     # Add the predictions and the true scores to the metric calculators
    #     spearman.add_batch(predictions=similarity,references=batch["score"])

    # # Print results
    # print("Pearson correlation coefficient : " + str(spearman.compute()))
